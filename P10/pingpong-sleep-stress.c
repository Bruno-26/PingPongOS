...
{
   // inicializacao do SO
   char strings [NUMTASKS][9], snum[3];
   int i,x;
  ...
  {
    strcpy (strings[i], "task ");
    sprintf(snum, "%i", i+2);
    strcat(strings[i],snum);
    // printf ("Creating task %d %s\n", i+2,strings[i]) ;
    printf ("Creating task %d \n", i+2) ;
    task_create (&task[i], Body, strings[i]) ;
  }
